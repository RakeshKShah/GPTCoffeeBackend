...
CASE_SUFFIX="$(date +%s)-$$"
TMP_DIR="/tmp/admin_sees_monthly_sales_aggregation-${CASE_SUFFIX}"
mkdir -p "$TMP_DIR"
...
# Then — monthly sales from /api/admin/sales matches aggregation derived from order history for current month
[ "$(cat "$TMP_DIR/orders.status")" = "200" ]
[ "$(cat "$TMP_DIR/sales.status")" = "200" ]
jq -e '.orders | type == "array"' "$ORDERS_RESPONSE" >/dev/null
jq -e '.monthly | numbers' "$SALES_RESPONSE" >/dev/null
EXPECTED_MONTHLY="$(jq -r '[.orders[] | select(((.createdAt | fromdateiso8601 | strftime("%Y-%m")) == (now | strftime("%Y-%m")))) | .total] | add // 0' "$ORDERS_RESPONSE")"
ACTUAL_MONTHLY="$(jq -r '.monthly' "$SALES_RESPONSE")"
[ "$ACTUAL_MONTHLY" = "$EXPECTED_MONTHLY" ]

echo "CODEVALID_TEST_ASSERTION_OK:admin_sees_monthly_sales_aggregation"
