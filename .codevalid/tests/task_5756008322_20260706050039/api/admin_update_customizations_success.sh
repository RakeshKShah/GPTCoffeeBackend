#!/usr/bin/env sh
set -eu
BASE_URL="${BASE_URL:-http://app:6713}"
CASE_SUFFIX="$(date +%s)-$$"
DB_DIR="/app/src/data"
DB_FILE="$DB_DIR/db.json"
BACKUP_FILE="/tmp/admin_update_customizations_success_${CASE_SUFFIX}_db.json"
RESPONSE_FILE="/tmp/admin_update_customizations_success_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/admin_update_customizations_success_${CASE_SUFFIX}.status"
LOGIN_FILE="/tmp/admin_update_customizations_success_${CASE_SUFFIX}_login.json"
PAYLOAD_FILE="/tmp/admin_update_customizations_success_${CASE_SUFFIX}_payload.json"
cleanup() {
  if [ -f "$BACKUP_FILE" ]; then
    cp "$BACKUP_FILE" "$DB_FILE"
  else
    rm -f "$DB_FILE"
  fi
  rm -f "$BACKUP_FILE" "$RESPONSE_FILE" "$STATUS_FILE" "$LOGIN_FILE" "$PAYLOAD_FILE"
}
trap cleanup EXIT

# Given
mkdir -p "$DB_DIR"
if [ -f "$DB_FILE" ]; then cp "$DB_FILE" "$BACKUP_FILE"; fi
cat > "$DB_FILE" <<'JSON'
{
  "users": [
    {
      "id": "buyer-sample",
      "name": "Maya Buyer",
      "email": "buyer@gptcoffee.test",
      "password": "buyer123",
      "role": "buyer"
    },
    {
      "id": "admin-001",
      "name": "Admin 001",
      "email": "admin-001@gptcoffee.test",
      "password": "adminpass123",
      "role": "admin"
    }
  ],
  "products": [
    {
      "id": "velvet-latte",
      "name": "Velvet Latte",
      "note": "Caramel, oat cream, dark espresso",
      "description": "Silky steamed milk folded into a double ristretto with a toasted caramel finish.",
      "price": 6.75,
      "strength": "Mellow",
      "gradient": "from-amber-300 via-orange-500 to-stone-900"
    }
  ],
  "customizations": [
    {"id":"cust-001","name":"Sugar Level","options":["None","Light","Normal","Extra"]}
  ],
  "orders": []
}
JSON
curl -sS -o "$LOGIN_FILE" -X POST "$BASE_URL/api/auth/login" \
  -H 'Content-Type: application/json' \
  --data '{"email":"admin-001@gptcoffee.test","password":"adminpass123"}'
TOKEN="$(jq -r '.token' "$LOGIN_FILE")"
[ "$TOKEN" != "null" ]

# When
cat > "$PAYLOAD_FILE" <<'JSON'
{"customizations":[{"id":"cust-001","name":"Sugar Level","options":["None","Light","Normal"]},{"id":"cust-002","name":"Milk Type","options":["Whole","Skim","Oat","Almond"]}]}
JSON
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X PUT "$BASE_URL/api/admin/customizations" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $TOKEN" \
  --data @"$PAYLOAD_FILE" > "$STATUS_FILE"

# Then
STATUS="$(cat "$STATUS_FILE")"
[ "$STATUS" = "200" ]
jq -e '.customizations | length == 2' "$RESPONSE_FILE" >/dev/null
jq -e '.customizations[0].id == "cust-001" and .customizations[0].name == "Sugar Level" and .customizations[0].options == ["None","Light","Normal"]' "$RESPONSE_FILE" >/dev/null
jq -e '.customizations[1].id == "cust-002" and .customizations[1].name == "Milk Type" and .customizations[1].options == ["Whole","Skim","Oat","Almond"]' "$RESPONSE_FILE" >/dev/null
jq -e '.customizations | length == 2' "$DB_FILE" >/dev/null
jq -e '.customizations[0].options == ["None","Light","Normal"]' "$DB_FILE" >/dev/null
jq -e '.customizations[1].name == "Milk Type"' "$DB_FILE" >/dev/null

echo "CODEVALID_TEST_ASSERTION_OK:admin_update_customizations_success"

# Cleanup
:
