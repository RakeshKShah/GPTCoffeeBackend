...
CASE_SUFFIX="$(date +%s)-$$"
PRODUCT_NAME="Latte ${CASE_SUFFIX}"
LOGIN_FILE="/tmp/admin_creates_product_price_conversion_login_${CASE_SUFFIX}.json"
RESPONSE_FILE="/tmp/admin_creates_product_price_conversion_${CASE_SUFFIX}.json"
cleanup_files() {
...
STATUS="$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST "$BASE_URL/api/admin/products" \
  ...
  --data "{\"name\":\"${PRODUCT_NAME}\",\"price\":\"4.75\"}")"

# Then
[ "$STATUS" = "201" ]
jq -e '.product.price == 4.75' "$RESPONSE_FILE" >/dev/null
jq -e '.product.price | type == "number"' "$RESPONSE_FILE" >/dev/null
CREATED_ID="$(jq -r '.product.id' "$RESPONSE_FILE")"

echo "CODEVALID_TEST_ASSERTION_OK:admin_creates_product_price_conversion"
