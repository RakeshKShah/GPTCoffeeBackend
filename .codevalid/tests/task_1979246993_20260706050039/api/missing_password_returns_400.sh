...
CASE_SUFFIX="$(date +%s)-$$"
USER_EMAIL="missing-password-${CASE_SUFFIX}@example.com"
USER_NAME="Jane Doe ${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/missing_password_returns_400_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/missing_password_returns_400_${CASE_SUFFIX}.status"
cleanup_files() {
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  ...
  --data "{\"name\":\"${USER_NAME}\",\"email\":\"${USER_EMAIL}\"}" > "$STATUS_FILE"
...
echo "CODEVALID_TEST_ASSERTION_OK:missing_password_returns_400"
