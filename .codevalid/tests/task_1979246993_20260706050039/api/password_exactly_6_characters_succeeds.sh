...
CASE_SUFFIX="$(date +%s)-$$"
USER_EMAIL="boundary-${CASE_SUFFIX}@example.com"
USER_NAME="Boundary Test ${CASE_SUFFIX}"
USER_PASSWORD="123456"
RESPONSE_FILE="/tmp/password_exactly_6_characters_succeeds_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/password_exactly_6_characters_succeeds_${CASE_SUFFIX}.status"
cleanup_files() {
...
if (user.password !== process.env.USER_PASSWORD) throw new Error('password not stored as submitted');
EOF

echo "CODEVALID_TEST_ASSERTION_OK:password_exactly_6_characters_succeeds"
