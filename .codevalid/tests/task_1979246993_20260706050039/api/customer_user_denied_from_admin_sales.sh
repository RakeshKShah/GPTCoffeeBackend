...
CASE_SUFFIX="$(date +%s)-$$"
TMP_DIR="/tmp/customer_user_denied_from_admin_sales-${CASE_SUFFIX}"
mkdir -p "$TMP_DIR"
...
echo "CODEVALID_TEST_ASSERTION_OK:customer_user_denied_from_admin_sales"

# Cleanup
# No persistent side effects; test only authenticates and verifies authorization.
