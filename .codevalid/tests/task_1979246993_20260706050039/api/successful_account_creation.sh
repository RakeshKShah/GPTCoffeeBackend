...
CASE_SUFFIX="$(date +%s)-$$"
USER_EMAIL="newuser-${CASE_SUFFIX}@example.com"
USER_NAME="John Doe ${CASE_SUFFIX}"
USER_PASSWORD="securePass123"
RESPONSE_FILE="/tmp/successful_account_creation_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/successful_account_creation_${CASE_SUFFIX}.status"
cleanup_files() {
...
if (!body.user || body.user.email !== process.env.USER_EMAIL) throw new Error('wrong email');
if (body.user.name !== process.env.USER_NAME) throw new Error('wrong name');
if (body.user.role !== 'buyer') throw new Error('wrong role');
if (!String(body.user.id || '').startsWith('buyer-')) throw new Error('wrong id format');
if ('password' in body.user) throw new Error('password should not be exposed');
EOF
node --input-type=module <<'EOF'
import { readFile } from 'node:fs/promises';
...
if (user.password !== process.env.USER_PASSWORD) throw new Error('stored password mismatch');
if (user.role !== 'buyer') throw new Error('stored role mismatch');
EOF

echo "CODEVALID_TEST_ASSERTION_OK:successful_account_creation"
