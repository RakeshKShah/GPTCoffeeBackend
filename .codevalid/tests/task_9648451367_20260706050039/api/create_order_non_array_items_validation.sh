...
CASE_SUFFIX="$(date +%s)-$$"
EMAIL="bob.wilson.${CASE_SUFFIX}@example.test"
PASSWORD="secret123"
RESPONSE_FILE="/tmp/create_order_non_array_items_validation_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/create_order_non_array_items_validation_${CASE_SUFFIX}.status"
LOGIN_FILE="/tmp/create_order_non_array_items_validation_login_${CASE_SUFFIX}.json"
cleanup_files() {
...
curl -sS -o /dev/null -X POST "$BASE_URL/api/auth/signup" \
  ...
  --data "{\"name\":\"Bob Wilson\",\"email\":\"${EMAIL}\",\"password\":\"${PASSWORD}\"}"
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST "$BASE_URL/api/orders" \
  ...
  --data '{"items":{"name":"Espresso","total":3}}' \
  > "$STATUS_FILE"
...
echo "CODEVALID_TEST_ASSERTION_OK:create_order_non_array_items_validation"
