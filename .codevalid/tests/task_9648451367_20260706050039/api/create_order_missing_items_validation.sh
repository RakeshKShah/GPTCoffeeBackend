...
CASE_SUFFIX="$(date +%s)-$$"
EMAIL="alice.brown.${CASE_SUFFIX}@example.test"
PASSWORD="secret123"
RESPONSE_FILE="/tmp/create_order_missing_items_validation_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/create_order_missing_items_validation_${CASE_SUFFIX}.status"
LOGIN_FILE="/tmp/create_order_missing_items_validation_login_${CASE_SUFFIX}.json"
cleanup_files() {
...
curl -sS -o /dev/null -X POST "$BASE_URL/api/auth/signup" \
  ...
  --data "{\"name\":\"Alice Brown\",\"email\":\"${EMAIL}\",\"password\":\"${PASSWORD}\"}"
...
echo "CODEVALID_TEST_ASSERTION_OK:create_order_missing_items_validation"
